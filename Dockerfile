...
# then log output of file /code/bin/rolld
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/rolld \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/rolld | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.21

COPY --from=build-env /code/build/rolld /usr/bin/rolld
