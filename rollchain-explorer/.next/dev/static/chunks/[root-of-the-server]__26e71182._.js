...
function Dashboard() {
    _s();
    const { networkStatus, blocks, transactions, loading, error, DISPLAY_DENOM: displayDenom, AVERAGE_GAS_PRICE: averageGasPrice } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$hooks$2f$useRollchainClient$2e$ts__$5b$client$5d$__$28$ecmascript$29$__["useRollchainClient"])();
    ...
    const latestHeightLabel = networkStatus.latestBlockHeight > 0 ? `#${networkStatus.latestBlockHeight.toLocaleString()}` : '—';
    const lastBlockTxLabel = blocks[0] ? `${blocks[0].txs} tx${blocks[0].txs === 1 ? '' : 's'}` : '—';
    const lastUpdatedLabel = networkStatus.latestBlockTime ? formatTimeAgo(networkStatus.latestBlockTime) : null;
    const statusLabel = networkStatus.catchingUp ? 'Syncing' : 'Healthy';
    const statusColor = networkStatus.catchingUp ? 'var(--accent-warning)' : 'var(--accent-success)';
    const gasLabel = `${averageGasPrice.toFixed(3)} ${displayDenom}`;
...
_s(Dashboard, "riLrA3uXBDDcaITl8hNMBz6lvwQ=", false, function() {
    return [
...
//# sourceMappingURL=%5Broot-of-the-server%5D__26e71182._.js.map